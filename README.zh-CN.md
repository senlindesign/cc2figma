...
### 它们如何协作
    ...
    "let's start"
        |
        v
    ① preflight ─── 一次性启动，加载 Token Map + 组件清单
        |
        v
    ② reference-interpreter ─── 可选：截图 / 参考图 → Design Brief → 用户确认
        |
        v
    ┌→ ③ component-rules ─── 搜索 Library，使用 Instance
    │       |
    │  ④ figma-style-binding ─── 绑定所有值到 Token，自动验证
    │       |
    └── 下一个 section（循环直到完成）
